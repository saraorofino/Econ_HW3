...
whales_numeric <- whales %>%
  mutate(income_num = ifelse(whales$income == "very_rich", 5,
                             ifelse(whales$income == "rich", 4,
                                    ifelse(whales$income == "middle", 3,
                                           ifelse(whales$income == "poor", 2,
                                                  ifelse(whales$income == "one_percent", 1, 0)))))) %>% 
...
inc_median <- median(whales_numeric$income_num) #median income bracket is 2 = poor 
...
Again assume the probability of voting yes the average of voting yes ($p = 0.714$). Use the median income bracket (poor), the median age bracket (to 50), and average NEP (38.366) to calculate willingness to pay using: 

$`r avg_vote` = `r round(int, digits = 4)` + `r round(inc_poor, digits=4)`(Income~Poor) + `r round(age_50, digits=4)`(Age~to~50) + `r round(nep, digits=4)`(NEP) - `r round(bid, digits=4)`(Bid) + `r round(risk, digits=4)`(60)$  
...
# Use the average NEP, median income (poor), and middle age bracket (to 50) to find the average bid

bid_sb <- (int + age_50 + inc_poor + (nep*nep_avg) + (risk*60) - avg_vote)/bid
